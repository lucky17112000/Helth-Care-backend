...
> {
    ...
    if (searchTerm && searchableFields && searchableFields.length > 0) {
      const searchConditions: Record<string, unknown>[] = searchableFields.map(
        (field) => {
          ...
          //!SECTION direct field search maeans not dot notaion or nested relation search
          const stringFilter: stringFilter = {
            contains: searchTerm,
            mode: "insensitive" as const,
          };
  ...
  //!SECTION parse range value
  private parseRangeFilter(
    value: Record<string, string | number>,
  ): PrismaNumberFilter | stringFilter | Record<string, unknown> {
